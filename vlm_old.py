...
def yi_vision_api(PROMPT='帮我把蓝色三角片放在钢笔上', img_path='temp/vl_now.jpg'):
    '''
...
def post_processing_viz(result, img_path, depth_frame, intr, check=False):
    '''
    视觉大模型输出结果后处理和可视化，并获取深度值
    check：是否需要人工看屏幕确认可视化成功，按键继续或退出
    depth_frame: Realsense的深度帧，用于获取深度值
    intr: 相机内参，用于像素坐标到相机坐标的转换
    '''
    ...
    # 将 intr 数组转换为 intrinsics 对象
    intrinsics = rs.intrinsics()
    intrinsics.fx = intr[0][0]
    intrinsics.fy = intr[1][1]
    intrinsics.ppx = intr[0][2]
    intrinsics.ppy = intr[1][2]

    # 获取深度值
    # 起点中心的深度值
    start_depth = depth_frame.get_distance(START_X_CENTER, START_Y_CENTER)
    # 将像素坐标转换为相机坐标
    x_cam_start, y_cam_start, z_cam_start = rs.rs2_deproject_pixel_to_point(intrinsics,
                                                                            [START_X_CENTER, START_Y_CENTER],
                                                                            start_depth)

    # 终点中心的深度值
    end_depth = depth_frame.get_distance(END_X_CENTER, END_Y_CENTER)
    # 将像素坐标转换为相机坐标
    x_cam_end, y_cam_end, z_cam_end = rs.rs2_deproject_pixel_to_point(intrinsics, [END_X_CENTER, END_Y_CENTER],
                                                                      end_depth)

    print(f"起点中心相机坐标：x: {x_cam_start:.3f}, y: {y_cam_start:.3f}, z: {z_cam_start:.3f}")
    print(f"终点中心相机坐标：x: {x_cam_end:.3f}, y: {y_cam_end:.3f}, z: {z_cam_end:.3f}")
    ...
    return x_cam_start, y_cam_start, z_cam_start, x_cam_end, y_cam_end, z_cam_end
